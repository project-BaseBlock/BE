...
public class TicketController {
    ...
    @Operation(summary = "내 티켓 목록(페이지네이션)", description = "로그인한 사용자의 발급 티켓 목록을 최신 발급순으로 반환")
    @GetMapping("/me")
    ...
    @Operation(summary = "내 티켓 단건 조회", description = "티켓 PK로 조회하며, 본인 소유 티켓만 접근 가능")
    @GetMapping("/{ticketId}")
    @PreAuthorize("hasAnyRole('USER','ADMIN','MASTER')")
    ...
    @Operation(summary = "예약ID로 생성된 내 티켓 조회(결제 직후 화면용)", description = "결제 완료 직후 예약ID로 생성된 최신 티켓 1건을 반환")
    ...
    @Operation(
            ...
            description = "보관지갑에 민팅된 티켓을 내 지갑으로 이관합니다. 성공 시 온체인 정보(txHash, tokenId 등)가 응답에 포함"
    )
